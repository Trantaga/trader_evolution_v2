"""WGAN-GP training script for 7-day (168-bar) BTC OHLC windows.

STEP 1 ONLY: this verifies the training loop runs stably end to end -- loss
stays finite, shapes line up, generated windows reconstruct into
structurally valid OHLC -- it does NOT tune for realism. Realism (matching
the reality-floor AUC target from `reality_floor/`) is Step 2.
...
N_GEN_STEPS = 300  # SHORT run: Step-1 verification only, not a realism run
LOG_EVERY = 20
...
def main() -> None:
    ...
    print(
        ...
        f"(window={WINDOW_LEN} bars = 7d, stride={WINDOW_STRIDE} bars)"
    )
    ...
    for step in range(1, N_GEN_STEPS + 1):
        ...
        if step == 1 or step % LOG_EVERY == 0:
            ...
            log.append(entry)
            print(
                f"step {step:4d}  critic_loss={entry['critic_loss']:+8.4f}  "
    ...
    print(
        ...
        f"{elapsed:.1f}s on {device} ({elapsed / N_GEN_STEPS * 1000:.1f} ms/gen-step)"
    )
    ...
    with open(RESULTS_DIR / "step1_loss_log.json", "w") as f:
        json.dump(
            {
                "device": str(device),
                "elapsed_sec": elapsed,
    ...
    with open(RESULTS_DIR / "step1_validity_check.json", "w") as f:
        json.dump(validity, f, indent=2)

    # a few sample paths for the README/plot
    sample_paths = c[:N_SAMPLE_PATHS].tolist()
    with open(RESULTS_DIR / "step1_sample_paths.json", "w") as f:
    ...
    try:
        ...
        ax.set_title("Step 1: sample generated close-price paths (untrained for realism)")
        ax.set_xlabel("bar (4h)")
        ax.set_ylabel("price (anchor = 1.0)")
        ax.legend()
        fig.tight_layout()
        fig.savefig(RESULTS_DIR / "step1_sample_paths.png", dpi=120)
        print(f"\nSaved sample-path plot to {RESULTS_DIR / 'step1_sample_paths.png'}")
    except ImportError:
